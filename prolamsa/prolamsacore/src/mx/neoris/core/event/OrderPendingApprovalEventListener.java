...
public class OrderPendingApprovalEventListener extends AbstractEventListener<OrderPendingApprovalEvent>
{
	...
	protected void onEvent(final OrderPendingApprovalEvent orderPendingApprovalEvent)
	{
		...
		if (orderModel.getStatus() != OrderStatus.ORDER_PENDING_APPROVAL_AND_PENDING_SAP_ID)
		{
		...
		if (baseStoreModel.getHSSFunctionaliatyEnabled() != null && baseStoreModel.getHSSFunctionaliatyEnabled() == true)
		{
			...
			if (hssOrderModel != null && hssOrderModel.getStatus() != OrderStatus.ORDER_PENDING_APPROVAL_AND_PENDING_SAP_ID)
			{
				final OrderProcessModel hssOrderProcessModel = (OrderProcessModel) getBusinessProcessService().createProcess(
						"orderPendingApprovalEmailProcess" + "-" + hssOrderModel.getCode() + "-" + System.currentTimeMillis(),
						"orderPendingApprovalEmailProcess");
