...
import mx.neoris.storefront.breadcrumb.impl.MyCompanyBreadcrumbBuilder;
import mx.neoris.storefront.controllers.ControllerConstants;
import mx.neoris.storefront.controllers.util.GlobalMessages;
import mx.neoris.storefront.forms.B2BBudgetForm;
import mx.neoris.storefront.forms.B2BCostCenterForm;
import mx.neoris.storefront.forms.B2BCustomerForm;
import mx.neoris.storefront.forms.B2BPermissionForm;
import mx.neoris.storefront.forms.validation.B2BBudgetFormValidator;
import mx.neoris.storefront.forms.validation.B2BPermissionFormValidator;
...
public class MyCompanyPageController extends AbstractSearchPageController
{
	...
	protected static final String REDIRECT_TO_USER_DETAILS = REDIRECT_PREFIX
			+ "/my-company/organization-management/manage-users";
	protected static final String REDIRECT_TO_USER_LIST = REDIRECT_PREFIX
			+ "/my-company/organization-management/manage-users";
	...
	{
		//NEORIS_CHANGE #111 Added method to populate b2b units checkboxes
		return populateB2BUnitsCheckBoxes(neorisFacade.getB2BUnitsForStore(getCurrentBaseStore()));
	}
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		//NEORIS_CHANGE #111 
   	if (b2BCustomerForm.getB2bUnits() == null || b2BCustomerForm.getB2bUnits().size() == 0)
   	{
   		GlobalMessages.addErrorMessage(model, "form.b2bcustomer.noB2BUnitSelected.error");
   		model.addAttribute(b2BCustomerForm);
   		return editUser(b2BCustomerForm.getUid(), model);
   	}
		...
		//NEORIS_CHANGE #111 
		List <B2BUnitData> b2bUnits = new ArrayList<B2BUnitData>();
		
		for(String b2bUnitUId : b2BCustomerForm.getB2bUnits())
		{
			b2bUnits.add(neorisFacade.getB2BUnitForUID(b2bUnitUId));
		}
		
		b2bCustomerData.setB2bUnits(b2bUnits);
		//NEORIS_CHANGE #111 
		b2bCustomerData.setUnit(b2bUnits.get(0));
		...
		try
		{
			...
			neorisFacade.updateB2BUnits(b2bCustomerData);
			neorisFacade.setBaseStore(b2bCustomerData);
		...
		return String.format(REDIRECT_TO_USER_LIST, urlEncode(b2bCustomerData.getUid()));
	}
	...
	public String editUser(final String user, final Model model) throws CMSItemNotFoundException
	{
		if (!model.containsAttribute("b2BCustomerForm"))
		{
			...
			//NEORIS_CHANGE #111 
			List <String> b2bUnitCodes =  new ArrayList<String>();
			
			for(B2BUnitData b2bUnitUId : neorisFacade.getB2BUnitsForUser(user))
			{
				b2bUnitCodes.add(b2bUnitUId.getUid());
			}
			
			b2bCustomerForm.setB2bUnits(b2bUnitCodes);
			...
			final Model model, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		b2bCustomerData.setActive(b2BCustomerForm.isActive());
	...
	//NEORIS_CHANGE #111 
	protected List<SelectOption> populateB2BUnitsCheckBoxes(final List <B2BUnitData> b2bUnitsDatas)
	{
		final List<SelectOption> selectBoxList = new ArrayList<SelectOption>();
		for (final B2BUnitData data : b2bUnitsDatas)
		{
			selectBoxList.add(new SelectOption(data.getUid(),data.getName()));
		}
