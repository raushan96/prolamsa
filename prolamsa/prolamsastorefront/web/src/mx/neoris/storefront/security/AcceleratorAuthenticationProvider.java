...
public class AcceleratorAuthenticationProvider extends CoreAuthenticationProvider
{
			...
			throws AuthenticationException
	{
		...
		// NEORIS_CHANGE #93
		// validate current base store matches user base store
		validateUserModelBaseStore(details);
	}
	
	// NEORIS_CHANGE #93
	private void validateUserModelBaseStore(final UserDetails details) throws InsufficientAuthenticationException
	{
		BaseStoreModel currentBaseStore = baseStoreService.getCurrentBaseStore();
		
		UserModel userModel = getUserService().getUserForUID(StringUtils.lowerCase(details.getUsername()));
		
		Set<BaseStoreModel> allowedBaseStores = ((B2BCustomerModel)userModel).getBaseStores();
		
		if(allowedBaseStores == null || !allowedBaseStores.contains(currentBaseStore))
		{
			throw new InsufficientAuthenticationException(messages.getMessage("checkout.error.invalid.accountType", "You are not allowed to login"));
		}
