...
<c:set var="b2bUnitBaseUrl" value="/client/searchAddress/list" />
...
<div id="addressb2bUnitResult" class="content">
</div>
...
function selectAddress(caption){	
     ...
     if (caption != jQuery("#deliveryAddressSelect").val() )
      {    	
     	jQuery("#deliveryAddressSelect").val(caption);	
     	jQuery("#deliveryAddressSelect").change();     	
...
{	
	...
	jQuery.ajax
	({
		url:  "<c:url value='/client/searchAddress/list.json'/>",
		type: 'POST',
		dataType: 'json',
		...
		{  
			if (data.status == 0)
           	{	
				...
				var options = $('#deliveryAddressSelect option');
				...
				if (data.units != null)
				{
					$.each(data.units, function(index) {					
						...
						if ( optionsValues.indexOf(data.units[index].code) != -1 ) {	
									...
									$("#addressb2bUnitResult").append("<td class='labelSmall col_center' width='30'><a href='#' id='selectAddressLink' onClick='selectAddress(\"" + data.units[index].code + "\")'>SELECT</a>&nbsp;</td>" +
