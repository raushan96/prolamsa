...
<c:set var="themeMsgKey" value="${not empty msgKey ? msgKey : 'search.page'}"/>
	...
	<c:if test="${paginationType eq 'pagination' && (searchPageData.pagination.numberOfPages > 1)}">
		<!-- NEORIS_CHANGE -->
		<pagination:prolamsaPageSelectionPaginationMobile searchUrl="${searchUrl}" searchPageData="${searchPageData}" numberPagesShown="${numberPagesShown}" themeMsgKey="${themeMsgKey}"/>
	</c:if>
