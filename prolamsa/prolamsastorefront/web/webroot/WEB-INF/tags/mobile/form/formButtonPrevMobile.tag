...
<button style="margin-right: 5px;" id="${id}" type="${type}" class="button ${css}" onclick='${onClick}' title="${title}" value="${value}" tabindex="${tabindex}" name="${name}" ${(disabled) ? 'disabled' : ''}>
    <img src="${themeResourcePath}/images/icon_triangle_left.gif" alt="" style="width: 10px; vertical-align:baseline;"> 
