...
<table class="grid_three_dimensions clear">
	...
	<tr>
		...
		<c:forEach items="${variants}" var="variant">
			...
			<td>
				<input type="hidden" class="${skusId} sku"  name="productQuantities[${loopIndex}].sku" id="productQuantities[${loopIndex}].sku" value="${variant.variantOption.code}" />
				<input type="${inputType}" class="sku-quantity" name="productQuantities[${loopIndex}].quantity" id="productQuantities[${loopIndex}].quantity" value="0" />
			</td>
