...
<div class="item_container_holder ${containerCSS}">
	<div class="title_holder">
		...
		<h2><spring:theme code="text.account.order.summaryTotals" text="Order Totals"/></h2>
	</div>
...
<%-- 		<format:price priceData="${cartData.subTotal}"/> --%>
			...
			<c:if test = "${order.galvametalWeight > 0}" >
			  <dt><spring:theme code="checkout.summary.galvametal"></spring:theme></dt>
			  <dd>${order.formattedGalvametalWeight} &nbsp;${order.sapWeightUnit}</dd>
			</c:if>
